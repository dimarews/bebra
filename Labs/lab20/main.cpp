...
int main()
{
    ...
    while (true) {
        int ExNum;
        cout << "   Choose number of task (enter 0 to exit)\n"
            << "   1) Array A with the size of N\n"
            << "   2) Integer L(>0) and an array in size of N\n"
            << "   3) Integer K(>0) and an array in size of N\n"
            << "   4) An array A with N dots \n"
            << "   5) An array A with N dots(N>2) \n";
        cin >> ExNum;
        if (ExNum == 1)
        {
            First();
